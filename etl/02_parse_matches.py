...
Writes five parquet files into data/processed/. Re-running overwrites them,
...
def parse_archive(zip_path: Path) -> dict:
    ...
    """
    matches, batting, bowling, squads = [], [], [], []
    people: dict[str, set] = {}          # keyed by id - names are not unique
    failures: list[tuple] = []
    ...
    with zipfile.ZipFile(zip_path) as archive:
        ...
        for index, name in enumerate(names, start=1):
            ...
            try:
                ...
                # This match's name-to-id map. Authoritative here, even where
                # two players share a name across the wider archive.
                ...
                # The team sheets. registry.people also contains umpires and
                # match referees, so it must NOT be used to define dim_player.
                # Squad membership is also the correct basis for "matches
                # played" - a selected player who never batted still played.
                for team_name, squad in (info.get("players") or {}).items():
                    for person in squad:
                        squads.append({
                            "match_id": match_id,
                            "team": team_name,
                            "player_id": registry.get(person),
                            "player_name": person,
                        })
                ...
                for number, one_innings in enumerate(data.get("innings", []),
                    ...
                    fielding_team = next((t for t in teams
                                          if t != batting_team), None)
    ...
    return {"matches": matches, "batting": batting, "bowling": bowling,
            "squads": squads, "people": people, "failures": failures}


def main() -> None:
    PROCESSED_DIR.mkdir(parents=True, exist_ok=True)

    all_matches, all_batting, all_bowling, all_squads = [], [], [], []
    ...
    print(f"   people in registry    {len(all_people):,}"
          f"   (includes umpires and referees)")
    print(f"   distinct names        {distinct_names:,}")
    print(f"   ids with >1 name      "
          f"{sum(1 for n in all_people.values() if len(n) > 1)}")
    print(f"   PLAYERS in squads     {squads_df['player_id'].nunique():,}")
    print(f"   squad rows            {len(squads_df):,}")
